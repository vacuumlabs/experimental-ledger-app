...
export function* startCountedSection(sectionLength: number): Interaction<StartCountedSection> {
    const P2_UNUSED = 0x00
    const response = yield send({
        p1: 0x09,
        p2: P2_UNUSED,
        data: Buffer.concat([uint64_to_buf(sectionLength.toString() as Uint64_str), uint8_to_buf(0 as Uint8_t)]),
